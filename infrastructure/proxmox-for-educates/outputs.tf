...
* Kubeconfig:   Saved locally as ./build/k8s_config.yaml
* Test Cluster: Run 'kubectl --kubeconfig ./build/k8s_config.yaml get nodes'

NEXT STEPS (MANUAL):
1. Install the Educates CLI on your local machine.
2. Execute the following command to deploy the training portal:
   
   educates cluster create --kubeconfig ./build/k8s_config.yaml --domain <your-domain.com>
